 ...
 */
export const createTable = pgTableCreator((name) => `frontend-gui_${name}`);
...
export const transcripts = createTable(
  ...
  (example) => ({
    ...
    conversationIndex: index("conversation_idx").on(example.conversation),
    speakerIndex: index("speaker_idx").on(example.speaker),
  }),
);
